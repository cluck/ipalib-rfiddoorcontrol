...
PLUGIN = 'rfiddoorcontrol'
PACKAGES = ['ipalib_rfiddoorcontrol']

with codecs.open(os.path.join('.', 'ipalib_rfiddoorcontrol', 'version.py')) as init:
    METADATA = dict(re.findall("__([A-Za-z][A-Za-z0-9_]+)__\s*=\s*'([^']+)'", init.read().decode('utf-8')))

if os.path.exists("README.md"):
    long_description = codecs.open("README.md", "r", "utf-8").read()
else:
    long_description = "See https://github.com/cluck/ipalib-rfiddoorcontrol"
...
class post_install(install_data):
	...
	shutil.copy(
            os.path.join(self.install_dir, 'ipalib/plugins/rfiddoorcontrol.py'),
            os.path.join(IPALIB_DIR, 'plugins/rfiddoorcontrol.py'),
        )

extra = {}

setup(
    name = 'ipalib-rfiddoorcontrol',
    packages=PACKAGES,
    version=METADATA['version'],
    description = 'RFID door control plugin for FreeIPA 4',
    long_description = long_description,
    url = 'https://github.com/cluck/ipalib-rfiddoorcontrol',
    ...
    entry_points={
        'console_scripts': [
            'rfiddoorctl = ipalib_rfiddoorcontrol.commands:rfiddoorctrl',
        ],
    },
    zip_safe=False,
    classifiers=[
